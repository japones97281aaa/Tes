...
-- [ ✔️ SHADOWS HUB - Bug Player/ALL Com Whitelist - Loop Infinito ]
-- [ 🔧 Por: ChatGPT - Integrado com sua GUI ]
...
-- ✅ Configuracoes
local whitelist = {"alodozhynn", "n9xgh", "Joao", "Pedro", "Maria"} -- nomes separados por virgula
local bugAllEnabled = false
local bugPlayerEnabled = false
local selectedPlayer = nil -- insira o nome exato ou parte do nome

-- 🔁 Função de bug (glitch forte)
local function bugPlayer(target)
    if not target or not target.Character then return end
    if table.find(whitelist, target.Name) then
        warn("[Whitelist] " .. target.Name .. " está na whitelist e não será bugado.")
        return
    end
    ...
    local crazyVector = Vector3.new(
        math.random(1e15, 9e15),
        math.random(1e15, 9e15),
        math.random(1e15, 9e15)
    )

    local gun = LocalPlayer.Backpack:FindFirstChild("Assault")
    local gunScript = gun and gun:FindFirstChild("GunScript_Local")
    ...
    local args = {
        ...
        crazyVector,
        hrp.Position,
        gunScript and gunScript:FindFirstChild("MuzzleEffect"),
        gunScript and gunScript:FindFirstChild("HitEffect"),
        0,
        0,
        { false },
...
-- 🔁 Loop de glitch (ALL e Player)
RunService.Heartbeat:Connect(function()
    if bugAllEnabled then
    ...
    if bugPlayerEnabled and selectedPlayer then
        for _, player in ipairs(Players:GetPlayers()) do
            if string.lower(player.Name):find(string.lower(selectedPlayer)) then
                bugPlayer(player)
            end
        end
    end
...
-- 🧠 GUI Botões TrollTab
TrollTab:AddSection({"Bug Glitch"})
TrollTab:AddToggle("Bug ALL (Infinito)", function(value)
    bugAllEnabled = value
end)

TrollTab:AddTextbox({
    Name = "Selecionar jogador para Bug",
    Text = "Digite o nome...",
...
TrollTab:AddToggle("Bug Player (Infinito)", function(value)
    bugPlayerEnabled = value
end)

TrollTab:AddTextbox({
    Name = "Whitelist (Separado por vírgula)",
    Text = table.concat(whitelist, ", "),
    Callback = function(input)
        whitelist = {}
        for name in input:gmatch("%s*([^,]+)%s*") do
            table.insert(whitelist, name)
        end
        print("[Whitelist Atualizada]", table.concat(whitelist, ", "))
    end
})


-- === GUI INTERFACE ABA TROLL === --

-- GUI estilo Coquette Hub para controle de bugs
...
-- UI usando TrollTab
TrollTab:AddSection({ "bugar server" })

TrollTab:AddTextbox({
    Name = "Adicionar à whitelist",
    Text = "",
    Placeholder = "Ex.: lo para (lolyta)",
    Callback = function(input)
        table.insert(whitelist, input)
        print("[+] Adicionado à whitelist:", input)
    end
})

TrollTab:AddButton({
    Name = "Verificar Whitelist",
    Callback = function()
...
TrollTab:AddToggle("bugar Todos (Loop)", function(value)
    bugAllEnabled = value
end)
...
TrollTab:AddButton({
    ...
    Callback = function()
        ...
        print("Jogadores atualizados:", table.concat(playerNames, ", "))
    end
})

TrollTab:AddToggle("bugar Jogador Selecionado", function(value)
    bugPlayerEnabled = value
end)
